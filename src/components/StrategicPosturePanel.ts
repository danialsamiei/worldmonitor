    ...
    return `
      <div class="posture-theater posture-expanded ${p.postureLevel}" data-lat="${p.centerLat}" data-lon="${p.centerLon}" title="${t('components.strategicPosture.clickToViewMap')}">
        ...
        <div class="posture-footer">
          ...
          ${isDesktopRuntime() ? `<button class="posture-deduce-btn" title="${t('common.analyzeWithAi')}" style="background: none; border: none; cursor: pointer; opacity: 0.7; font-size: 1.1em; transition: opacity 0.2s; margin-inline-start: auto;" data-theater='${escapeHtml(JSON.stringify(p))}'>\u{1F9E0}</button>` : ''}
        </div>
      </div>
    `;
  }
