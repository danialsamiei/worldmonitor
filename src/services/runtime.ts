...
const DEFAULT_WEB_API_URL = 'https://api.gadr.alefba.dev';
...
function isWorldMonitorWebHost(hostname: string): boolean {
  return hostname === 'gadr.alefba.dev'
    || hostname === 'www.gadr.alefba.dev'
    || hostname.endsWith('.gadr.alefba.dev');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://gadr.alefba.dev';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.gadr.alefba.dev');
  } catch {
    return false;
  }
}
